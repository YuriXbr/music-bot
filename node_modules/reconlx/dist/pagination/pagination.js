...
const pagination = async (options) => {
    ...
    const numberTravel = async () => {
        if (pageTravelling.has(author.id))
            return channel.send("escreva `end` para cancelar a navegação");
        const collector = channel.createMessageCollector({
            filter: (msg) => msg.author.id === author.id,
            time: 30000,
        });
        const numberTravelMessage = await channel.send(`${author.tag}, você tem 30s, mande um numero do chat para ir até a pagina. Escreva \`end\` para sair.`);
